...
## 19.14–19.29 停机根治（2026-09-13 → 2026-09-15，用户实测通过）

> 本节是这段排查的**结论**。中间的错误假设与逐帧取证过程已删除；代码注释里引用的小节号
> （19.14/15/18/19/21/22/23/24/25/28/29）在下文都有对应条目。

### 停机只有三类（`vm.rs` 里 8 个 `halted = true` 点）

| 类 | 机理 | 状态 |
|---|---|---|
| **A** 处理器 `Err` | 指令处理器返回 Err（栈下溢、未实现命令返回形态错） | **已修**（2 处）+ 容错 |
| **B** PC 失步 | 解码落到非指令边界（含读档后的场景错配） | **已修**（读档那处）+ 容错 |
| **C** 结构性 | 如探针单场景启动时的 `return_at_root_frame` | 真实游戏不触发，无需处理 |

### A 类：两处 `str stack underflow`（19.14 / 19.15 / 19.18 / 19.19）

- 站点一 `seen6416:948`：`namae` 走"未实现 form 保命回退"时推的是**空元素** ⇒ 紧随其后的
  `PROPERTY` 走 `elm.is_empty()` 分支压 `Int 0` ⇒ 下一条 `ASSIGN`（`right_form=FM_STR`）`pop_str` 下溢。
- 站点二 `seen6416:949`：脚本自建的 `namae_global[elm_array][1]` 与 parent-slot 同形，
  被 `try_parent_slot_property` **截胡**且不压值。
- **关键事实**：`FM_STRREF = 23`（不是 Element）；`namae(str)` 返回的是**引用**；
  跨场景用户命令尾部 `read_flag_no` 故意不被消费。
- **修复**：
  1. `namae`(108) 返回**槽位引用** `Value::Element([ELM_GLOBAL_NAMAE_GLOBAL, elm_array, slot])`
     （slot 由全角字母算出，702 = 26 + 26×26）；
  2. `try_parent_slot_property` 开头守卫：namae 两张表（106/107）直接 `return false`。
- **验证**：`clannad_probe --scene seen6416 --frames 40000` 不再产生 `engine_halt.log`。
- ⚠️ **19.16 / 19.17 的弯路**：把 `namae` 的返回值做成**字符串**会卡在标题画面（新游戏进不去），
  已 revert；**不要再试**——它必须返回元素。

### 容错 + 记账（19.21，含 19.22 / 19.23 / 19.25）

- **覆盖**：处理器 `Err`；`CD_NONE` 填充（19.22）；未知操作码（19.23）；跨场景返回 pc 落在填充上。
- **记账**：STATE 的 `recovered` 字段 + MCP 工具 `get_recovered()`，
  内容为 `scene/line/pc/opcode/错误文本`（去重计数）。**一次游玩即可拿到整份问题清单**。
- **开关**：`SIGLUS_VM_STRICT=1` 恢复"一遇错就停"。
- **保护**：同一站点重复 >1000 次交回致命路径（防解码失步死循环）。
- **19.25（重要）**：返回 pc 的合法性校验必须放在 `self.stream = saved.stream` **之后**——
  放在流恢复之前会拿被调方的字节流去"修正"调用方的返回地址，**自己造出失步**。

### B 类：读档后场景级返回错配（19.28 / 19.29）

**根因（三条叠加）**：
1. 场景进入路径**先切 `current_scene_no` 到目标场景**再创建/标记帧 ⇒ **调用方帧被贴上被调场景的标签**；
2. 存档写入端给**每一帧都写"当前场景"**（所有帧同值），读取端还**直接丢弃**该字段；
3. **`scene_stack` 完全不进存档**。
⇒ 读档后"调用方帧属于哪个场景"丢失：场景级返回既无边界记录、又不知 pc 属于哪个场景，
走同场景路径 `exec_return`，把外层场景的 pc 用到当前场景 ⇒ 失步 ⇒ `cd_none`。

**修复（三处配合）**：
1. 两处跨场景进入（user-cmd / farcall）在切场景**之前** `caller.scene_no = self.current_scene_no`；
2. 存档**写/读每帧自己的场景**；
3. 读档会话中，若场景栈为空、调用方 pc 在**当前流**不像指令、但在**它自己场景**里像指令
   ⇒ 从场景缓存取回该场景流，补一个 `SceneExecFrame`，让返回走 `return_from_scene`。
   旧存档（标签无信息量）另有兜底 `infer_scene_for_pc()`：找"该 pc **唯一**合法"的场景，**不唯一就不猜**。

**安全性设计**：重建**只在读档会话**生效（`POST_LOAD_RETURN_BOUNDARY_PENDING`，
在读档**完成之后**置位，`jump_to_scene_name` 会清掉）。必须如此——`seen6416` 回放里
`jump_to_scene_name` 也会清空场景栈，只判断"场景栈为空"会误触发并**导致回归**。

**验证**：
- 新存档：`--load-slot 3 --advance-every 6` 从 `seen6900 line=51` 起，**回到 `seen0414` 并推进到 line 200**；
  清单仅 1 条 `scene_boundary_rebuilt caller_scene=17 (seen0414)`；`--skip-to-choice --frames 20000` **不再停机**；
- 回归门槛 `seen6416 --frames 40000`：**不产生 `engine_halt.log`**；
- 用户旧存档：slot 1 / slot 2 读档 + skip 正常；**slot 0 仍会停**（旧档标签无信息量且 pc 在多个场景
  都"像指令"，按设计不猜）⇒ 旧档用新构建**另存**一次即可，或从 slot 1/2 继续。

### 19.20 存档与消息框（用户实测反馈）

- **存档没有丢**：`savedata_zh/0000,0001,0002.sav` 完好；被改写的只有 `read/global/config.sav`
  （正常回写，非零率 ~88%，与旧副本同尺寸）。备份：`diagnostics\save_backup_2026-09-15\`。
  端口里**唯一会删档**的路径是 `SYSCOM::DELETE_SAVE → delete_slot → remove_game_file`（不进回收站）。
- **存档列表**：`original_save::save_dir` 优先用 `savedata_zh`；`clannad_ctl` 可离线验证
  （`save_count/save_used/save_slots`）；曾出现的"列表为空"是**引擎实例内存状态**，非文件问题。
- **待办**：游戏调用系统消息框（Yes/No）时，端口的独立窗口**可能开在游戏窗口后面**，
  表现为"有提示音、看不到对话框、skip 不返回"。修法：给该窗口置顶+请求焦点，或改为游戏内绘制。

### 不要重复的做法（已验证无效或会回归）

| 做法 | 结果 |
|---|---|
| 把 `namae` 返回值做成字符串 | 卡标题画面（19.16→19.17 已 revert） |
| 每个调用都写 `callee.return_override` 并在返回时优先用它 | 回归：`_system_language:350 pc=0x2306` 下溢 |
| 只按"场景栈为空"触发边界重建 | 回归（`jump_to_scene_name` 也会清空场景栈） |
| 在流恢复**之前**校验跨场景返回 pc | 自造失步 |
| 放宽 `at_cross_scene_return_boundary` 为"场景不匹配也算" | 1003 次 `wrong call depth` |
| 用行号/文本替换脚本批量改代码 | 三次改坏源码；一律用带唯一锚点的编辑 |

### 自测与观测规程（后续沿用）

| 用途 | 命令 / 要点 |
|---|---|
| **构建身份校验** | `engine_state.log` 首行 `[BUILD]` 的 `size` 必须与刚构建的 release exe 一致；窗口关掉 ≠ 进程退出（必要时重启机器清残留实例） |
| 读档复现/验收 | `clannad_ctl --project <游戏根> --scene seen0414 --load-slot N --skip-to-choice --frames 6000`（看 `choose.final` 的 steps/halted/halt_reason/recovered） |
| **回归门槛（改行为必跑）** | `clannad_probe --project <游戏根> --scene seen6416 --frames 40000 --click --click-every 12` → **不得产生 `engine_halt.log`**（`line=1673 halted=true` 是探针假象） |
| 造新存档 | `clannad_ctl --scene seen0414 --verify-sl --save-at-frame 1500 --slot 3 --frames 3000`（看 `{"event":"save"}` 与 `sl.roundtrip ok:true`） |
| 选择回归 | `--choose-index 0` → 第 1 句、`--choose-index 1` → 第 2 句 |
| 追踪开关 | `SIGLUS_TRACE_CALL_RETURN_PC=1`（调用/返回 pc）、`SIGLUS_TRACE_CALLPC_ALL=1`（每次 return pc 写入）、`SIGLUS_TRACE_VM[_SCENE/_PC]`（逐指令）、`SIGLUS_VM_STRICT=1`（回到"一遇错就停"） |

### 当前状态（2026-09-15）

- 行为代码：`siglus_rs` `e5bd51e`；release exe `size=59270289`、`mtime=09/15 17:59:30`；
...
## 19.31 `advance` / `skip` 返回游戏内日期（仅变化时）（2026-09-15）

**日期来源**：CLANNAD 把「月/日/星期」放在全局整型表的固定下标 `1109/1110/1111`；
检测器现在统一为 **`SceneVm::in_game_date()`**（`vm.rs`），引擎桥与 `clannad_ctl` 共用一份实现
（`clannad_ctl` 原来的本地函数改为转调）。
