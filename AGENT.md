...
## 19.20 用户首次跑通后的三件事（2026-09-15）：无数据丢失 / 守卫收窄 / 消息框遮挡

用户第一次跑到 `948` 之后的内容，报告三件事：
「中途出现以前没有的选项」「Windows 提示音但没有弹窗、skip 不能正确返回」「以前的 save 都没了」。

### 1. 存档**没有丢**（已核实，并已备份）

端口把存档放在**游戏目录**（`original_save::save_dir` → `savedata_zh`，见 `original_save.rs:709-717`）。

| 文件 | 大小 | 时间 | 结论 |
|---|---|---|---|
| `0000.sav` / `0001.sav` / `0002.sav` | 276422 / 312478 / 312013 | 9/8、9/9、9/13 | **完好未动** |
| `read.sav` / `global.sav` / `config.sav` | 169119 / 34988 / 990 | **9/15 15:02（本次运行回写）** | 大小与旧副本一致；非零字节 88.8% / 88.4%；`config.sav` 与旧副本**逐字节相同** |

⇒ 本次运行只是像正常游戏那样**回写了全局状态**，没有清空、没有配置重置、没有删除任何槽位。
已备份到 `diagnostics\save_backup_2026-09-15\`（槽位文件 MD5 与磁盘一致）。
**「save 都没了」应是游戏内读档列表的显示问题**，不是文件丢失。

### 2. 「多出来的选项」是预期行为，但守卫确实过宽（已收窄）

停机修好后，游戏会走过 `948/949` 进入**以前从未到达过的内容** ⇒ 出现新选项本身是正常的。

但复查发现 `try_parent_slot_property` 的守卫用的是 `is_global_indexed_list_head()`，
它包含 `GLOBAL_INT_LIST_FORMS = [25..32, 137]` —— **剧情旗标列表**！
那会让 **全游戏** 的旗标/列表元素读取在 `CD_PROPERTY` 路径上改变行为
（以前走 parent-slot 拿到默认值，现在读到真实值）⇒ **完全可能改变分支判定**，与用户观察吻合。

已收窄为**只针对 namae 两张名字表**（106/107，只存显示名，不碰剧情旗标）：

```rust
if elm.first().copied().is_some_and(|head| {
    head == codes::ELM_GLOBAL_NAMAE_LOCAL || head == codes::ELM_GLOBAL_NAMAE_GLOBAL
}) { return false; }
```

**收窄后重跑同一个 4 分钟复现：仍然 `no underflow`，仍然推进到 line 1673** ⇒ 收窄不影响修复效果。
提交 `b9ae8a3`；release 重建 `size=59211770`、`mtime=09/15 15:15:16`。

### 3. 提示音 + 无弹窗 + skip 不返回 = 端口的**桌面消息框**（下一个真 bug）

游戏走到新内容后调用了 `SYSCOM` 的系统消息框（Yes/No）。端口实现是**另开一个 OS 窗口**：
`DesktopMessageBoxWindow::new`（`desktop_messagebox.rs:104-140`）用 `WindowAttributes::default()`
建窗，**没有请求焦点、没有置顶**；`pump_desktop_messagebox_requests`
（`siglus_engine.rs:3769-3790`）只在创建失败时回 cancel（这条路径是对的，不会永久卡死）。

⇒ 窗口很可能**建在游戏窗口后面**，用户只听到提示音、看不到对话框，VM 一直等按钮结果
⇒ 「skip 不能正确返回」。这与存档无关，也不是本次修复引入的——只是以前走不到这段代码。

**待办**：给消息框窗口加 `set_visible(true)` + 请求焦点/置顶（或在游戏内渲染该 modal）。
